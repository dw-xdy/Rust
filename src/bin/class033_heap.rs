...
//     }
//
//     // In the heap, I don't care about any elements other than the first element
//     // also, heap just make sure the biggest one is top in heap.
//     while let Some(number) = my_heap.pop() {
//         // .pop() returns Some(number) if a number is there, None if not. It pops from the front
//         println!(
//             "Popped off {number}. Remaining numbers are: {:?}",
